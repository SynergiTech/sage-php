...
class OpeningBalanceJournalsApi
{
     ...
     *
     * @param  int|null $itemsPerPage Returns the given number of Journal Opening Balances per request. (optional, default to 20)
     * @param  int|null $page Go to specific page of Journal Opening Balances (optional, default to 1)
    ...
    {
        list($response) = $this->getOpeningBalanceJournalsWithHttpInfo($itemsPerPage, $page, $attributes, $contentType);
        return $response;
    }
    ...
    {
        $request = $this->getOpeningBalanceJournalsRequest($itemsPerPage, $page, $attributes, $contentType);
    ...
    {
        return $this->getOpeningBalanceJournalsAsyncWithHttpInfo($itemsPerPage, $page, $attributes, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    {
        list($response) = $this->getOpeningBalanceJournalsKeyWithHttpInfo($key, $attributes, $contentType);
        return $response;
    }
    ...
    {
        $request = $this->getOpeningBalanceJournalsKeyRequest($key, $attributes, $contentType);
    ...
    {
        return $this->getOpeningBalanceJournalsKeyAsyncWithHttpInfo($key, $attributes, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     *
     * @param  \SynergiTech\Sage\Model\PostOpeningBalanceJournals $openingBalanceJournals openingBalanceJournals (required)
    ...
    public function postOpeningBalanceJournals(
        \SynergiTech\Sage\Model\PostOpeningBalanceJournals $openingBalanceJournals,
        string $contentType = self::contentTypes['postOpeningBalanceJournals'][0]
    ): \SynergiTech\Sage\Model\OpeningBalanceJournal
    {
        list($response) = $this->postOpeningBalanceJournalsWithHttpInfo($openingBalanceJournals, $contentType);
        return $response;
    }
     ...
     * Creates a Opening Balance Journal
     *
     * @param  \SynergiTech\Sage\Model\PostOpeningBalanceJournals $openingBalanceJournals (required)
     * @param  string $contentType The value for the Content-Type header. Check self::contentTypes['postOpeningBalanceJournals'] to see the possible values for this operation
     *
     * @throws ApiException on non-2xx response or if the response body is not in the expected format
     * @throws InvalidArgumentException
    ...
    {
        $request = $this->postOpeningBalanceJournalsRequest($openingBalanceJournals, $contentType);
    ...
    {
        return $this->postOpeningBalanceJournalsAsyncWithHttpInfo($openingBalanceJournals, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    ): Request
    {
        ...
        // verify the required parameter 'openingBalanceJournals' is set
        if ($openingBalanceJournals === null || (is_array($openingBalanceJournals) && count($openingBalanceJournals) === 0)) {
            throw new InvalidArgumentException(
                'Missing the required parameter $openingBalanceJournals when calling postOpeningBalanceJournals'
            );
        }
        ...
        // for model (json/xml)
        if (isset($openingBalanceJournals)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($openingBalanceJournals));
            } else {
                $httpBody = $openingBalanceJournals;
            }
