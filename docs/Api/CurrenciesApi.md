...
```php
getCurrencies($showLegacyId, $itemsPerPage, $page, $attributes): \SynergiTech\Sage\Model\Base[]
```
...
try {
    $result = $apiInstance->getCurrencies($showLegacyId, $itemsPerPage, $page, $attributes);
    print_r($result);
} catch (Exception $e) {
...
```php
getCurrenciesKey($key, $showLegacyId, $attributes): \SynergiTech\Sage\Model\Base
```
...
try {
    $result = $apiInstance->getCurrenciesKey($key, $showLegacyId, $attributes);
    print_r($result);
} catch (Exception $e) {
